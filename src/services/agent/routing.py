...
_BASE_RULE = (
    ...
    "   - If the user asks for a broad type like 'outdoor lights' or 'indoor lights' WITHOUT specifying a subcategory (e.g. 'wall lights', 'gate lights', 'bollard lights', 'solar lights', 'street lights'), call `GetCategoriesDatabase` to retrieve all subcategories, then present ONLY the relevant subcategories (filtering by 'outdoor' or 'indoor') and ask which specific type they want. Only call `SearchProductsDatabase` once the user specifies a concrete subcategory, OR if they have a strong long-term memory preference for one.\n"
