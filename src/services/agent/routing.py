...
_BASE_RULE = (
    ...
    "RULES:\n"
    "1. ALWAYS use tools to query the database before answering. If the user's request is broad (e.g. 'show me products', 'outdoor lights', 'indoor lights') AND you do not have long-term memory specifying their preferred category, DO NOT call `SearchProductsDatabase`. Instead, call `GetCategoriesDatabase` to find available categories, and ask a clarifying question to narrow down their choice.\n    HOWEVER, if you DO have long-term memory (e.g. user prefers garden lights), you MUST bypass `GetCategoriesDatabase`, and directly use `SearchProductsDatabase` with that explicit `category` to save them time.\n"
